#!/usr/bin/env python3
"""seed_double_bottom_h1_winners.py — ship the H1+D1 sweep winners as
PAPER deployments at 0.05% risk + $50 max-$. 4-week observation window.

H1 winners (from sweep_new_strategies_h1_d1.py, 2026-05-08):

  1. XAUUSD H1 double_bottom RR 1:1 long (no RSI div)
     OOS PF 2.10, R +0.43, WR 59%, DD 2.0%, recov 34d, n_test=17.
     Adds gold H1 layer.

  2. UK100.cash H1 double_bottom RR 1:1 long (no RSI div)
     OOS PF 2.08, R +0.43, WR 57%, DD 1.1%, recov 50d, n_test=21.
     LOWEST DD in entire H1 sweep. Adds new ticker (UK100).

Both at conservative 0.05% per trade + $50 max-$ for 4-week paper run.
"""
...
CELLS = [
    ("double_bottom", "XAUUSD", "H1", 100.0,
     ...
     DoubleBottomParams(
         ...
         target_R_mult=1.0,    # RR 1:1 — winner on H1
         max_wait_bars=20, long_only=True,
     ),
     "double_bottom XAUUSD H1 RR_1:1 long. OOS PF 2.10, R +0.43, "
     "WR 59%, DD 2.0%, recov 34d, n_test=17. Adds gold H1 layer "
     "to portfolio. Different from existing donchian_55 H1 cell — "
     "double-bottom fires on REVERSAL, donchian on BREAKOUT, so "
     "they hit different market states."),

    ("double_bottom", "UK100.cash", "H1", 1.0,
     DoubleBottom,
     DoubleBottomParams(
         pivot_window=5, min_separation=8, max_separation=60,
         band_pct=0.003, rsi_period=14,
     ...
     "double_bottom UK100 H1 RR_1:1 long. OOS PF 2.08, R +0.43, "
     "WR 57%, DD 1.1% (LOWEST in H1 sweep), recov 50d, n_test=21. "
     "Adds UK100 — new ticker not previously in portfolio. UK index "
     "diversification (you have GER40 EU exposure, US100/500 US, "
     "JP225/HK50 Asian, but no UK)."),
]
...
def main() -> int:
    print("Seeding 2 H1 sweep winners as PAPER\n"
          "  - XAUUSD H1 double_bottom RR_1:1 long\n"
          "  - UK100.cash H1 double_bottom RR_1:1 long\n"
